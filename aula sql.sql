...
INSERT INTO venda (
data_venda, produto, categoria, valor_unitario,
...
#5
SELECT cliente, valor_unitario
FROM venda
WHERE valor_unitario > 3000;

#6
SELECT produto, categoria, valor_unitario, quantidade, cidade
FROM venda
...
##1:
SELECT cliente, produto, categoria, valor_unitario, quantidade
FROM venda
...
SELECT estado, cidade,
 ROUND(SUM(quantidade * valor_unitario),2) AS total_vendido
FROM venda 
...
##3:
SELECT cliente,
ROUND (AVG(valor_unitario * quantidade),2) AS media_gasto
FROM venda
WHERE categoria = 'Informática'
GROUP BY cliente
